...
function MatchCard({ match, isSelected, onClick, lang, theme, hasStream }: { match: Match; isSelected: boolean; onClick: () => void; lang: string; theme: 'light' | 'dark'; hasStream?: boolean }) {
  ...
  return (
    ...
    >
      ...
      {/* Footer Info */}
      <div className={cn("mt-5 pt-3 flex flex-wrap gap-2 items-center justify-between px-2 w-full border-t", theme === 'dark' ? 'border-white/10' : 'border-gray-200')}>
         {match.date && (
           <div className={cn("flex items-center gap-2", theme === 'dark' ? 'text-gray-400' : 'text-gray-500')}>
             ...
             <span className={cn("text-[11px] font-medium mr-2 rtl:border-r ltr:border-l rtl:pr-2 ltr:pl-2 hidden sm:inline", theme === 'dark' ? 'border-gray-700 text-gray-500' : 'border-gray-300 text-gray-400')}>
         ...
         {(isLive || hasStream) && (
            <div className="flex items-center gap-1.5 shrink-0 bg-red-600/20 text-red-500 text-xs px-2.5 py-1 rounded-full border border-red-500/20 shadow-[0_0_10px_rgba(220,38,38,0.2)]">
              <span className="w-1.5 h-1.5 bg-red-500 rounded-full animate-pulse shadow-[0_0_5px_rgba(239,68,68,0.8)]"></span>
              <span className="font-bold tracking-wide uppercase">{lang === 'ar' ? 'شاهد الان' : 'WATCH'}</span>
            </div>
         )}
      </div>
    </article>
          ...
          filteredMatches.map((match, index) => {
            ...
            <Fragment key={match.id}>
              <MatchCard 
                ...
                lang={lang}
                theme={theme}
                hasStream={hasServerStream(match)}
